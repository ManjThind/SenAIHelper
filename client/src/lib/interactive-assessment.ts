...
export async function getNextInteraction(
  ...
  const response = await anthropic.messages.create({
    ...
    messages: [
      ...
      {
        role: 'user',
        content: `Child's age: ${childAge}
Previous questions and responses:
${previousQuestions.map((q, i) => `Q: ${q}\nA: ${previousResponses[i] || 'No response'}`).join('\n')}

Based on these interactions, provide the next appropriate question and analyze the responses so far.`
      }
    ],
  });
  ...
  try {
    ...
    return {
      ...
      analysis: {
        ...
        comprehension: result.analysis.comprehension,
